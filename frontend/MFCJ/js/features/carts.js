...
export class Cart {
   ...
   * @param {string} productHandle - Product handle/ID
   * @param {Element} button - Button element
   */
  async handleQuickAdd(productHandle, button) {
    // Add loading state
    ...
    try {
      ...
      this.showToast('Added to cart', 'Product has been added to your cart.', 'success');
      ...
      this.showToast('Error', 'Failed to add product to cart. Please try again.', 'error');
   ...
   */
  removeItem(itemId) {
    const itemIndex = this.items.findIndex(item => (item._id || item.id) === itemId);
  ...
  updateQuantity(itemId, quantity) {
    const item = this.items.find(item => (item._id || item.id) === itemId);
  ...
  getSubtotal() {
    return this.items.reduce((total, item) => {
      // Handle both API response format (item.product) and local format
  ...
  updateDrawerContent() {
    ...
    console.log('updateDrawerContent - isEmpty:', this.isEmpty(), 'items:', this.items.length);
  ...
  renderCartItems() {
    return this.items.map(item => {
      ...
      const escapedId = this.escapeHtml(itemId);
      const escapedTitle = this.escapeHtml(product.title);
      const escapedVariant = this.escapeHtml(item.selectedColor || item.variant || '');
      
      // Get image from product.image_url array or product.image
      let imageUrl = '';
      ...
      const sanitizedImage = this.sanitizeImageUrl(imageUrl);
      
      return `
      <div class="cart-item" data-cart-item="${escapedId}">
        <div class="cart-item__image">
          <img src="${sanitizedImage}" alt="${escapedTitle}" width="80" height="80" loading="lazy" onerror="this.src='data:image/svg+xml,%3Csvg xmlns=%22http://www.w3.org/2000/svg%22 width=%2280%22 height=%2280%22%3E%3Crect fill=%22%23ddd%22 width=%2280%22 height=%2280%22/%3E%3Ctext x=%2250%25%22 y=%2250%25%22 text-anchor=%22middle%22 dy=%22.3em%22 fill=%22%23999%22 font-size=%2212%22%3ENo Image%3C/text%3E%3C/svg%3E'">
        </div>
        <div class="cart-item__details">
          <h4 class="cart-item__title">${escapedTitle}</h4>
          ${escapedVariant ? `<p class="cart-item__variant">${escapedVariant}</p>` : ''}
          <p class="cart-item__price">${this.formatPrice(product.price)}</p>
  ...
  openDrawer() {
    ...
    // Fetch latest cart from API and update UI
   ...
   */
  async refreshCartFromAPI() {
    try {
      const response = await cartModel.getCart();
      const cart = response.cart;
      
      // Update local items from API cart
      ...
      // Still try to show something from localStorage
      this.updateUI();
    }
  }
