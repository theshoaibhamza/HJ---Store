...
export class CartController {
   ...
   */
  renderCartItem(item) {
    // Backend returns 'product' not 'productId'
    const product = item.product || item.productId;
    
    if (!product) {
      console.error('Product data missing in cart item:', item);
      return '';
    }
  ...
  async handleCheckout(event) {
    ...
    // Show alert for now since checkout page doesn't exist
    const goToContact = await this.showConfirmDialog(
      'Checkout is under construction. Would you like to contact us to complete your order?',
