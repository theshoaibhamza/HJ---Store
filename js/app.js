...
class App {
   ...
   */
  initWishlist() {
    this.wishlistItems = Utils.storage.get('wishlist') || [];
    ...
    if (wishlistToggle) {
      wishlistToggle.addEventListener('click', (e) => {
        e.preventDefault();
        this.showWishlistToast();
      });
    }

    // Wishlist add buttons on product cards
    document.addEventListener('click', (e) => {
      const wishlistBtn = e.target.closest('[data-wishlist-add]');
      if (wishlistBtn) {
        e.preventDefault();
        const productId = wishlistBtn.dataset.wishlistAdd;
        this.toggleWishlistItem(productId, wishlistBtn);
      }
    });
  }
  ...
  toggleWishlistItem(productId, btn) {
    ...
    if (index > -1) {
      ...
      this.showToast('Removed from Wishlist', 'Item removed from your wishlist');
    } else {
      this.wishlistItems.push(productId);
      btn?.classList.add('is-active');
      this.showToast('Added to Wishlist', 'Item added to your wishlist');
    }
    Utils.storage.set('wishlist', this.wishlistItems);
    this.updateWishlistCount();
  }
  ...
  showWishlistToast() {
    const count = this.wishlistItems.length;
    if (count === 0) {
      this.showToast('Wishlist Empty', 'Add items to your wishlist by clicking the heart icon on products');
    } else {
      this.showToast('Your Wishlist', `You have ${count} item${count > 1 ? 's' : ''} in your wishlist`);
    }
  }
  ...
  initProductForm() {
    ...
    const wishlistBtn = form.querySelector('[data-wishlist-add]');
  ...
  async handleAddToCart(productData, button) {
    ...
    try {
      ...
      if (cartCount) {
        ...
        cartCount.textContent = currentCount + productData.quantity;
      }
